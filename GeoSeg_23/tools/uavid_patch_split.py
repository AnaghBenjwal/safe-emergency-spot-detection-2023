...
Safe = np.array([255, 255, 255]) # label 1 (1)
NotSafe = np.array([0, 0, 0]) # label 2 (0)
...
def label2rgb(mask):
    ...
    mask_rgb[np.all(mask_convert == 0, axis=0)] = NotSafe
    mask_rgb[np.all(mask_convert == 1, axis=0)] = Safe
...
def padifneeded(image, mask):
    ...
    pad = albu.PadIfNeeded(min_height=2160, min_width=4096)(image=image, mask=mask)

...
def label2rgb(mask):
    ...
    mask_rgb[np.all(mask_convert == 1, axis=0)] = np.array([255,255,255]) #Safe
    ...
    mask_rgb[np.all(mask_convert == 0, axis=0)] = np.array([0,0,0])  #NotSafe
    mask_rgb[np.all(mask_convert == 255, axis=0)] = np.array([255,255,255]) #Boundary
    return mask_rgb
...
def rgb2label(label):
    ...
    label_seg[np.all(label == Safe, axis=-1)] = 1
    ...
    label_seg[np.all(label == NotSafe, axis=-1)] = 0

...
#     mask_rgb[np.all(mask_convert == 255, axis=0)] = [255, 255, 255]
...
#     mask_rgb[np.all(mask_convert == 255, axis=0)] = [233, 193, 133]
...
def uavid2rgb(mask):
    ...
    mask_convert = mask[np.newaxis, :, :]
    mask_rgb[np.all(mask_convert == 0, axis=0)] = [0, 0, 0]
    mask_rgb[np.all(mask_convert == 1, axis=0)] = [0, 128, 0]
    mask_rgb[np.all(mask_convert == 255, axis=0)] = [255, 255, 255]
...
def main():
    ...
    for seq in seqs:
        print('inside seq for loop')
        img_paths = []
        output_path = os.path.join(args.output_path, str(seq), 'Labels')
        if not os.path.exists(output_path):
            os.makedirs(output_path)
        for ext in ('*.tif', '*.png', '*.jpg'):
            img_paths.extend(glob.glob(os.path.join(args.image_path, 'Images', ext)))
        img_paths.sort()
        # print(img_paths)
        for img_path in img_paths:
            ...
            with torch.no_grad():
                ...
                for input in tqdm(dataloader):
                    ...
                    raw_predictions = model(input['img'].cuda(config.gpus[0])).cpu()
            ...
            if args.dataset == 'uavid':
                print('output mask to RGB UAVId conversion')
                output_mask = uavid2rgb(output_mask)
            else:
                output_mask = output_mask
            assert img_shape == output_mask.shape
            # output_path = "fig_results\uavid\unetformer_r18\Images\Labels\img_%count.png"%count
            cv2.imwrite(r"trial_data\results\result" + str(count) + ".jpg", output_mask)
            count+=1
